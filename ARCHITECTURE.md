...
**Model Context Protocol** (`cmd/mcp.go`, `cmd/mcp_tools.go`, `cmd/mcp_resources.go`, `cmd/mcp_apps.go`): uses **mcpkit** v0.1.24 (split packages: `core/`, `server/`) + **ext/ui** v0.1.24 (MCP Apps). Single-struct registration (`srv.Register`), workspace middleware, per-tool timeouts, `StructuredResult` for typed output, error handler for session lifecycle, EventStore for Streamable HTTP reconnection, SSE grace period, streaming progress via `EmitContent`, server-side JSON Schema validation. Exposes **13 tools** (11 core + 2 preview) and **7 browsable resources** for reading deck content. Transports: Streamable HTTP (default), SSE (`--sse`), or stdio (`--stdio`). `--deck-root` sets the local workspace root. See [docs/MCP.md](docs/MCP.md).
...
**MCP Apps** (`cmd/mcp_apps.go`): `preview_deck` and `preview_slide` register as app tools via the `io.modelcontextprotocol/ui` extension. Hosts that support the extension render slide previews inline as iframes. Both tools go through `d.Build()` for a full self-contained deck; `preview_slide` additionally injects an init script to open on the requested slide. Preview HTML is built on demand through the workspace on every resource read — no HTML cache. This ensures authz is always enforced (critical for multi-tenant hosted deployments).
