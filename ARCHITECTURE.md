...
**Model Context Protocol** — One server path: `slyds mcp`, hand-written, built on **mcpkit** v0.5.1 + **ext/ui** v0.5.1 (MCP Apps) + **ext/auth** v0.5.1. Exposes **13 tools** (11 core + 2 preview), **9 resources** (7 data + 2 preview), and **completions** for deck names and slide positions. Optimistic versioning via `expected_version` / `expected_deck_version`. `--allow-origin '*'` for tunnel/remote deployments. Transports: Streamable HTTP (default), SSE (`--sse`), stdio (`--stdio`). See [docs/MCP.md](docs/MCP.md).
