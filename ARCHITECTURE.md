...
**Model Context Protocol** — One server path: `slyds mcp`, hand-written, built on **mcpkit** v0.7.1 + **ext/ui** v0.7.1 (MCP Apps) + **ext/auth** v0.7.1. Exposes **13 tools** (11 core + 2 preview), **9 resources** (7 data + 2 preview), and **completions** for deck names and slide positions. With `--analyze-cmd`, also registers `analyze_deck`, a SEP-2663 task-capable tool (**ext/tasks**) that runs the command once per slide; `slyds ws analyze` is its tasks-aware client. Optimistic versioning via `expected_version` / `expected_deck_version`. `--allow-origin '*'` for tunnel/remote deployments. Transports: Streamable HTTP (default), SSE (`--sse`), stdio (`--stdio`). See [docs/MCP.md](docs/MCP.md).

**Proto MCP** — removed in the mcpkit v0.5.1 upgrade. `proto/slyds/v1/` still holds the annotated RPC definitions, but the generated tree, `SlydsServiceImpl`, the parity E2E tests, and the `slyds mcp-proto` subcommand are gone. `protoc-gen-go-mcp` lives in mcpkit *experimental* and is pinned to the mcpkit v0.2.x API, so keeping the dependency would have held the module three minor versions back. The definitions and the revival procedure are in [proto/README.md](proto/README.md); the constraint is in [CONSTRAINTS.md](CONSTRAINTS.md).
