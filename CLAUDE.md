...
- **App Bridge speaks MCP Apps `2026-01-26`** — since mcpkit v0.7.0 the bridge JS that `ui.InjectAppBridge` adds to previews reads `hostCapabilities` from the `ui/initialize` result, sends `notifications/message` instead of `ui/log` and `ui/notifications/request-teardown` instead of `ui/teardown`, and ignores messages from any window other than the parent. `slyds-app.js` only uses `MCPApp` events and handlers, so it needed no change. A custom host written against the old bridge output needs updating.
- **`analyze_deck` is a task (SEP-2663)** — off unless `slyds mcp --analyze-cmd` (or `SLYDS_ANALYZE_CMD`) is set. The handler validates input, then returns `GoAsyncResult` when the client declared tasks. mcpkit reruns it in a goroutine with a `TaskContext`, and `notifications/progress` is silently dropped there, so per-slide progress goes through `setTaskStatusMessage`, which writes the task store directly because mcpkit v0.7.0 has no public call for it. The analyzer runs in its own process group so cancel kills its children too. `slyds ws analyze` is the only tasks-aware client; editors get the sync fallback. See [docs/MCP.md](docs/MCP.md#long-running-analysis-tasks).
...
| mcpkit | v0.7.0 | TypedTool, typed handler contexts, schema validation, sampling, elicitation |
| mcpkit/ext/auth | v0.7.0 | JWT validation, PRM, RFC 8414 proxy, scope enforcement, OAuth discovery |
| mcpkit/ext/tasks | v0.7.0 | SEP-2663 tasks — `analyze_deck` task runtime, task store |
| mcpkit/ext/ui | v0.7.0 | MCP Apps — display modes, App Bridge, theme adaptation, bidirectional tools |
| servicekit | v0.1.5 | HTTP helpers, memory event store |
| oneauth | v0.1.37 | JWT + OAuth discovery, indirect via mcpkit/ext/auth |

Go toolchain floor is **1.26.6**, the same as mcpkit v0.7.0's floor. It clears
the reachable stdlib advisories that 1.26.5 still carries.
