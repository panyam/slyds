...
| mcpkit | v0.7.1 | TypedTool, typed handler contexts, schema validation, sampling, elicitation |
| mcpkit/ext/auth | v0.7.1 | JWT validation, PRM, RFC 8414 proxy, scope enforcement, OAuth discovery |
| mcpkit/ext/tasks | v0.7.1 | SEP-2663 tasks — `analyze_deck` task runtime, task store |
| mcpkit/ext/ui | v0.7.1 | MCP Apps — display modes, App Bridge, theme adaptation, bidirectional tools |
| servicekit | v0.1.5 | HTTP helpers, memory event store |
| oneauth | v0.1.37 | JWT + OAuth discovery, indirect via mcpkit/ext/auth |

Go toolchain floor is **1.26.6**, the same as mcpkit's floor. It clears
