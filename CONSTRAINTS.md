...
**Why**: protogen is pinned to the mcpkit v0.2.x API while mcpkit ships v0.7.x, so depending on it strands the whole module on an old, unsupported mcpkit. That is exactly what blocked the v0.5.1 upgrade until the proto path was cut. The hand-written MCP path (`slyds mcp`) is the only production path.
