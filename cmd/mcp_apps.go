...
// opening on a specific slide. Used by template resource handlers.
...
// extracted from the URI params by the TemplateHandler. mcpkit v0.2.6
// auto-generates a concrete fallback resource for hosts that don't support
// template variable substitution (VS Code, Claude Desktop).
//
// Preview HTML is NOT cached — each resource read builds fresh through
// the workspace. This ensures authz is always checked and previews are
// never stale.
func registerAppTools(srv *server.Server) {
	...
	ui.RegisterAppTool(srv, ui.AppToolConfig{
		...
		TemplateHandler: func(ctx mcpcore.ResourceContext, uri string, params map[string]string) (mcpcore.ResourceResult, error) {
